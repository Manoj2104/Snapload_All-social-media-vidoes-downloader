...
def get_opts(is_yt=True, strategy="ios"):
    ...
    if is_yt:
        # We switch clients based on strategy
...
def download_video_task(job_id, url, format_type, quality):
    ...
    fmt = "bestaudio/best" if format_type == "audio" else f"bestvideo[height<={res}]+bestaudio/best"
    
    for s in ["ios", "tv", "android"]:
        try:
            opts = get_opts(is_yt, s)
            opts.update({"format": fmt, "outtmpl": os.path.join(DOWNLOAD_DIR, f"{job_id}.%(ext)s"), "merge_output_format": "mp4"})
            ...
            final_file = os.path.join(DOWNLOAD_DIR, f"{job_id}.{'mp3' if format_type=='audio' else 'mp4'}")
